...
仓库包含 GitHub Actions 发布流程。推送 `v0.1.0` 这样的 tag 后，会构建 ad-hoc GitHub 包并上传：
...
持久自动挂载路径会保持 plist 稳定，把所有会变化的 profile 参数放在 `/Library/Application Support/ZeroFSManager/Profiles/<profile-id>/zerofs.toml` 和 root-only `zerofs.env`。修改 endpoint、bucket、prefix、挂载目录、端口、缓存、配额或凭据后，在 App 里点击“应用并重启 LaunchDaemon”让 launchd 重新读取配置。
