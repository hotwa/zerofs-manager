...
このリポジトリには GitHub Actions のリリースワークフローがあります。`v0.1.2` のような tag を push すると、ad-hoc GitHub パッケージをビルドして次のファイルをアップロードします。
...
GitHub 開発版 DMG をインストールするときは、まず `ZeroFS Manager.app` を `/Applications` にドラッグまたはコピーし、その後 `/Applications` から起動してください。永続 sudo LaunchDaemon profile は安定したインストール済み App パスを必要とするため、マウントした DMG から直接起動しないでください。
...
永続的な自動マウント経路では plist を安定させ、変更される profile パラメータは `/Library/Application Support/ZeroFSManager/Profiles/<profile-id>/zerofs.toml` と root-only `zerofs.env` に保存します。インストールまたは更新時に、レビュー済み sudo スクリプトはユーザーがインストールした `zerofs` バイナリを同じ root-owned profile runtime ディレクトリにコピーし、LaunchDaemon はユーザーが書き換え可能な PATH エントリではなく、その固定コピーを実行します。endpoint、bucket、prefix、マウント先、ポート、キャッシュ、クォータ、認証情報、または ZeroFS バイナリを変更した後は、App の「適用して LaunchDaemon を再起動」で launchd に設定を再読み込みさせます。
